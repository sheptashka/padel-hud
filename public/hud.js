...
function saveHudCache(s) {
  ...
  try { localStorage.setItem(HUD_CACHE_KEY, JSON.stringify(s)); } catch (_) {}
}

function loadHudCache() {
  try { const raw = localStorage.getItem(HUD_CACHE_KEY); return raw ? JSON.parse(raw) : null; }
  catch (_) { return null; }
}
...
function setBg(bg) {
  hud.classList.remove("bg-transparent","bg-black","bg-white","bg-chroma-green","bg-chroma-purple","bg-blue","bg-red","bg-yellow");
  const map = { transparent:"bg-transparent", black:"bg-black", white:"bg-white", chroma_green:"bg-chroma-green", chroma_purple:"bg-chroma-purple", blue:"bg-blue", red:"bg-red", yellow:"bg-yellow" };
...
function updateMetaTournament(s) {
  if (!meta) return;
  if ((s.mode ?? "tournament") !== "tournament") { meta.style.display = "none"; meta.textContent = ""; return; }
  const N = Number(s.maxPoints ?? 11);
  const a = Number(s.a3 ?? 0);
  const b = Number(s.b3 ?? 0);
  let left = N - (a + b);
  if (!Number.isFinite(left)) left = 0;
  if (left === 1) { meta.textContent = "остался розыгрыш: 1"; meta.style.display = "inline-flex"; return; }
  if (left <= 0) {
    ...
    meta.style.display = "inline-flex"; return;
  }
...
function updateServeIndicator(s) {
  ...
  if (serveAEl) serveAEl.classList.toggle("show", currentServer === "A");
  if (serveBEl) serveBEl.classList.toggle("show", currentServer === "B");
}
...
function applyTournamentState(s) {
  ...
  if ($("teamA")) $("teamA").textContent = s.teamA ?? "TEAM A";
  if ($("teamB")) $("teamB").textContent = s.teamB ?? "TEAM B";
  if ($("a3")) $("a3").textContent = String(s.a3 ?? 0);
  if ($("b3")) $("b3").textContent = String(s.b3 ?? 0);
  if ($("totalA")) $("totalA").textContent = String(totals.totalA);
  if ($("totalB")) $("totalB").textContent = String(totals.totalB);
...
function applyState(s) {
  ...
  const tournamentEl = $("hudTournament");
  const tennisEl = $("hudTennis");
